...
- **Community / Verified Student Insights** at `/community` (2026-05-14) — gated by `NEXT_PUBLIC_ENABLE_COMMUNITY`. Mock-data-only first cut: every university and program in the catalog gets a community group (rich content for 8 hand-picked unis, auto-generated stubs for the rest). 4 tabs (Reviews · Groups · Questions · Campus Responsibles), filter bar, group-chat modal. **Redesigned 2026-06-11:** sidebar removed (feed is full width; stats + top-unis widgets deleted), hero illustration replaced with a self-hosted campus video (`public/videos/community-hero.mp4`, Pexels free license), "what you can do" + verify cards moved to the top, and the subscription honestly narrowed to its one perk — DMing campus responsibles (group-chat writing is verification-gated only). Subscription gating uses a cookie (`edfind_community_subscribed`) with a floating dev toggle until Stripe is wired — `useSubscription()` hook + `<SubscriberOnly>` wrapper isolate the source so the swap is one-file when billing arrives. Fake data + group generator live in `lib/community/`. Full spec in `docs/features/community.md`.
...
- **Housing & cost of living** (2026-06-09) — city- and university-level student-housing sections on university + program detail pages, gated by `NEXT_PUBLIC_ENABLE_HOUSING`. AI-researched data in `housing_cities` / `housing_universities` (sources + `researched_on` + draft/published `status`; public-read on published only). Produced by a fan-out research workflow (one web-researching subagent per city + university) → `scripts/import-housing.mjs`. **Full catalog covered: 42 cities / all 58 universities, published.** Spec in `docs/features/housing.md`.
- **University hero images** (2026-06-10) — real, freely-licensed campus photos (Wikimedia Commons / CC / public-domain) shown as a hero banner on university + program pages with required attribution. `universities.hero_image_url` + `hero_image_credit` + `hero_image_source_url`; `<UniversityHero>` uses a plain `<img>` (CSP `img-src` already allows `https:`). Sourced by the `university-image-research` workflow → `scripts/import-university-images.mjs`. All 58 universities have an attributed image.

- **Feedback round 2** (2026-06-10) — a batch of product changes:
  - **Quiz schema v6** (`ANSWERS_VERSION` 4→6). `current_situation` reworked (added `graduating_soon`, split recent-grad/gap-year, writable "Other"). New **`institution`** question — searchable Turkish-university picker (`lib/quiz/turkish-universities.ts` + `components/quiz/institution-combobox.tsx`) with free-text fallback and a legend that adapts to the situation. The structured field-of-study picker was **dropped**: required free-text `study_background` replaces it and the AI **infers `field_of_study` (now nullable)** from it. **GPA is compound** (`gpa_scale` + `exact_gpa` + `gpa_range`). `english_level` reframed to CEFR-anchored options (selecting `certified` reveals an `english_exam_score` write-in, v6). `budget_per_year` gained `tuition_free` (hard matcher filter). `career_goal` expanded + writable "Other". GPA is a **two-step journey** (pick scale → it collapses → enter value, v6). Matcher (`lib/ai/index.ts`) + parser (`lib/ai/parse-query.ts`) prompts updated to match. Spec: `docs/features/profile-quiz.md`.
  - **Quiz sound toggle removed** — the in-quiz mute button was confusing; sound now defaults **off** (`lib/sound/sound-context.tsx`), `components/quiz/sound-toggle.tsx` deleted.
  - **Shortlist nav removed** — saving = tracking since the 2026-06-09 merge, so the redundant "Shortlist" header link is gone (the `/shortlist` route stays as the `interested` view + compare driver).
  - **Footer + legal/transparency pages** — `components/site-footer.tsx` in the root layout; new `/terms`, `/privacy`, and **`/technical-report`** (a public "how EdFind works" page that renders the **live** matcher + parser system prompts via exported `SYSTEM_PROMPT` constants — single source of truth). Shared shell: `components/legal/legal-page.tsx`.
  - **Housing → summary + expandable** (`<details>`) and **euro equivalents** on every non-EUR figure via `eurEquivalent()` (`components/housing/housing-section.tsx`).

- **Feedback round 3** (2026-06-11) — header-search modal moved to the top of the viewport (blurred dead zone removed); homepage "How it works" + `/start` copy generalized (no model name-drops outside `/technical-report` and `/privacy`); **quiz v7** (`ANSWERS_VERSION` 6→7, 13→14 questions): the study question split into `study_background` ("what did you study" — undergrad only) and a new required free-text **`desired_study`** ("what do you want to study" — explicitly open-ended: a field, a precise topic, or a vague interest all valid; AI infers `field_of_study` primarily from it). Matcher prompt updated; `/search` leaves it null (the verbatim query carries the signal).
- **Document studio (AI CV & cover letters)** (2026-06-11) — section at the bottom of `/applications` (shares `NEXT_PUBLIC_ENABLE_APPLICATIONS`): drafts a CV or program-specific cover letter with DeepSeek (strictly anti-fabrication — missing facts become bracketed placeholders), editable/copy/download-as-`.md`. New `application_documents` table (RLS no-policy, service-role only; `application_id` ON DELETE SET NULL); actions in `app/applications/document-actions.ts`; prompts in `lib/ai/generate-document.ts`, rendered live on `/technical-report`. Spec: `docs/features/application-documents.md`.

- **Feedback round 4** (2026-06-11) — presentation prep:
  - **Premium tier + demo paywall** — two tiers now: Free (catalog, community read for everyone — `/community` no longer requires auth; verified-email writing) and **Premium €5/month** (AI matching, application tracking + document studio, DMs to campus responsibles). `/quiz`, `/search`, `/applications`, `/shortlist`, `/compare` gate via `requirePremium()` → `/premium?next=…`, where an "Activate Premium" button simulates checkout (cookie `edfind_premium`; deliberately works in production — it's a demo). Community subscription delegates to the same flag; `/community/upgrade` redirects to `/premium`. Spec: `docs/features/premium.md`.
  - **Document studio PDF export** — dependency-free print-to-PDF (`lib/documents/export.ts`) with a fill-the-[placeholders] panel before export (PDF-only substitution).
  - **Community hero = video background** — busier campus clip (Pexels #30614645, trimmed to 12s/720p ≈1.3 MB via ffmpeg) as full-bleed background with a left→right white gradient, copy on the left. Mock first names removed from group boxes + chat modal; group cards/modal link to university & program pages; top upgrade strip removed (Premium pitch lives on the responsible-message buttons only).
  - **`SelectPill`** (`components/ui/select-pill.tsx`) — custom rounded, animated dropdown replacing native `<select>` in community + catalog filter bars.
  - **Quiz preview modal** (`components/welcome/quiz-demo.tsx`) — auto-playing 5-step quiz demo with typewriter free-text answers, opened from the hero's "Watch the quiz in action" link. Spec: `docs/features/welcome-tour.md` § sibling.

- **Self-playing demo route `/demo`** (2026-06-14) — public, ungated, auto-playing walkthrough of the **whole journey** for the presentation video: home → quiz (14 questions) → AI matching → results → save → applications tracker. Renders the **real** components throughout (`components/demo/` drives `QuestionScreen`, `QuizLoadingScreen`, `HeroMatchCard`/`SiblingMatchCard`, and the real `ApplicationsClient`); the homepage hero is re-rendered faithfully in `demo-home.tsx` (the real one is a DB-backed server component). Only the persona auto-fill, a deterministic curated top-3 (real catalog programs, real logos in `public/demo-logos/`), and the curated applications data are scripted — no auth, premium, DeepSeek key, or DB. A fake cursor (`demo-cursor.tsx`) does the "click quiz" / "click save" beats; `HeroMatchCard` gained a `saveSlot` render-prop so the demo's save doesn't fire a server action. `?chrome=0` hides the demo badge/Replay. `node scripts/record-demo.mjs` records it to `recordings/edfind-quiz-ai-demo.mp4` (Playwright headless → ffmpeg, ~59s, 1080p). Spec: `docs/features/demo.md`.

- **Turkish localization** (2026-07-20) — full bilingual UI (EN default / TR) via **next-intl v4 without i18n routing** (ADR 0007): locale lives in the `edfind_locale` cookie, set by a header `LanguageSwitcher` (EN|TR pill) through a Server Action; URLs unchanged. Messages are per-feature namespace JSONs in `messages/{en,tr}/` (aggregated by `messages/{locale}/index.ts`); DB values (country/city/language/field) localized at render via `lib/i18n/data-labels.ts` helpers. "EdFind" and university/program names are never translated; quiz option labels are overlaid via the `quiz` namespace with fallback to `lib/quiz/schema.ts` English (values + ANSWERS_VERSION untouched). The matcher writes rationales in the active locale (`ui_language` in the profile payload) and the free-text parser accepts Turkish queries; AI CVs/cover letters and mock community UGC deliberately stay English. Spec + glossary + tone rules: `docs/features/i18n.md`.

Next: Stripe / tier billing. See `docs/architecture.md` for the full phased plan.
