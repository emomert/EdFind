...
- **Feedback round 2** (2026-06-10) — **quiz schema v5** (`ANSWERS_VERSION` 5): reworked `current_situation` (+ writable Other), new searchable Turkish-university `institution` question, dropped the field-of-study picker for a **required** `study_background` free-text with AI-inferred (nullable) `field_of_study`, **compound GPA** (`gpa_scale`/`exact_gpa`/`gpa_range`), CEFR-anchored `english_level`, `tuition_free` budget, expanded `career_goal` (+ Other). Quiz sound toggle removed (sound off by default). Redundant "Shortlist" nav link removed. New **footer** + `/terms` + `/privacy` + `/technical-report` (renders the live matcher/parser prompts). Housing → **summary + expandable** with **euro equivalents** on non-EUR figures. See `docs/features/profile-quiz.md` and `docs/features/housing.md`.
