...
- **Phase 9** (2026-05-11) — four features in one drop:
  ...
  - **9.2 Shortlist + compare** at `/shortlist` and `/compare?ids=…`. `SaveButton` lives on results / program / university pages. **Merged into the applications tracker (2026-06-09):** saving creates an `applications` row at status `'interested'`; `/shortlist` is the `'interested'` view; `saved_programs` is deprecated (backfilled, not yet dropped); the header "Shortlist" nav link was removed 2026-06-10.
...
- **Feedback round 2** (2026-06-10) — **quiz schema v6** (`ANSWERS_VERSION` 6): reworked `current_situation` (+ writable Other), new searchable Turkish-university `institution` question, dropped the field-of-study picker for a **required** `study_background` free-text with AI-inferred (nullable) `field_of_study`, **two-step GPA journey** (`gpa_scale` → `exact_gpa`/`gpa_range`), CEFR-anchored `english_level` (+ `english_exam_score` write-in for IELTS/TOEFL when `certified`), `tuition_free` budget, expanded `career_goal` (+ Other). Quiz sound toggle removed (sound off by default). Redundant "Shortlist" nav link removed. New **footer** + `/terms` + `/privacy` + `/technical-report` (renders the live matcher/parser prompts). Housing → **summary + expandable** with **euro equivalents** on non-EUR figures. See `docs/features/profile-quiz.md` and `docs/features/housing.md`.
